...
export default class RadioButton extends PureComponent {
    ...
    getNameIcon = () => {
        ...
        if (buttonType === 'Radio') {
            return isActive ? { size, name: 'radio-enable', color: GlobalStore.color.gradientOrange1 } : {
                size,
                name: 'radio-disable',
                color: GlobalStore.color.neutral4
            };
        } else {
            return isActive ? { size, name: 'checkbox-enable', color: GlobalStore.color.gradientOrange1 } : {
                size,
